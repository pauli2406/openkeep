import { getDocumentTypeDefinition } from "../document-intelligence.registry";
import type { TypeSpecificExtractor } from "./types";

const definition = getDocumentTypeDefinition("portfolio_statement");

export const portfolioStatementExtractor: TypeSpecificExtractor = {
  documentType: "portfolio_statement",
  promptFocus:
    "Extract institution, statement date, depot or account reference, and current portfolio total value. Prefer portfolio value or total assets over movement totals.",
  extractFields: (input, helpers) => ({
    issueDate: helpers.findDateByLabels(input, definition.issueDateLabels ?? []),
    amount: helpers.findAmount(input),
    currency: helpers.findCurrency(input),
    referenceNumber: helpers.findReferenceNumber(input, definition.referenceNumberLabels ?? []),
    correspondentName: helpers.findCorrespondentCandidate(input),
  }),
  refineFields: (input, fields) => {
    const portfolioValue = readFinancialAmount(input.parsed.text, [
      /(?:portfolio value|depotwert|gesamtwert|total assets|depotbestand)[:\s]*?(\d{1,3}(?:\.\d{3})*,\d{2}|\d+(?:,\d{2})?)\s*(EUR|€|USD|CHF)/i,
    ]);
    if (!portfolioValue) {
      return fields;
    }

    return {
      ...fields,
      amount: portfolioValue.amount,
      currency: portfolioValue.currency,
    };
  },
};

function readFinancialAmount(text: string, patterns: RegExp[]): { amount: number; currency: string } | null {
  const normalized = text.replace(/\u00a0/g, " ");
  for (const pattern of patterns) {
    const match = normalized.match(pattern);
    if (match?.[1] && match[2]) {
      return {
        amount: Number(match[1].replace(/\./g, "").replace(",", ".")),
        currency: match[2] === "€" ? "EUR" : match[2].trim().toUpperCase(),
      };
    }
  }
  return null;
}
