...
export class LlmAnswerProvider implements AnswerProvider {
  ...
  async answer(input: {
    ...
    return {
      status: "answered",
      answer: answer.trim(),
      reasoning: `Generated by ${this.llmService.getProviderInfo()?.provider ?? "LLM"} based on ${context.citations.length} grounded excerpt(s)${context.lowConfidence ? " with low retrieval confidence" : ""}.`,
