...
describe("preload bridge contract", () => {
  it("exposes named operations over fixed channels", async () => {
    ...
    await bridge.session.clearOfflineCopy({ profileId: "profile-id" });
    ...
    expect(invoke).toHaveBeenNthCalledWith(8, DESKTOP_CHANNELS.profilesList);
    expect(invoke).toHaveBeenNthCalledWith(9, DESKTOP_CHANNELS.profilesActivate, {
      profileId: "profile-id",
    });
    expect(invoke).toHaveBeenNthCalledWith(10, DESKTOP_CHANNELS.profilesRename, {
      profileId: "profile-id",
      label: "Home",
    });
    expect(invoke).toHaveBeenNthCalledWith(11, DESKTOP_CHANNELS.profilesRemove, {
      profileId: "profile-id",
    });
    expect(invoke).toHaveBeenNthCalledWith(12, DESKTOP_CHANNELS.importsPick);
    expect(invoke).toHaveBeenNthCalledWith(13, DESKTOP_CHANNELS.importsPending);
    expect(invoke).toHaveBeenNthCalledWith(14, DESKTOP_CHANNELS.importsAssign, {
      batchId: "batch-id",
      profileId: "profile-id",
    });
    expect(invoke).toHaveBeenNthCalledWith(15, DESKTOP_CHANNELS.importsConsume);
    expect(invoke).toHaveBeenNthCalledWith(
      16,
      DESKTOP_CHANNELS.importsReportCreated,
      { documents: [{ documentId: "document-id", name: "invoice.pdf" }] },
    );
    expect(invoke).toHaveBeenNthCalledWith(17, DESKTOP_CHANNELS.saveRequest, {
      kind: "document-original",
      documentId: "11111111-1111-4111-8111-111111111111",
    });
    expect(invoke).toHaveBeenNthCalledWith(18, DESKTOP_CHANNELS.watchFoldersList);
    expect(invoke).toHaveBeenNthCalledWith(19, DESKTOP_CHANNELS.watchFoldersAdd);
    expect(invoke).toHaveBeenNthCalledWith(
      20,
    ...
    expect(invoke).toHaveBeenNthCalledWith(26, DESKTOP_CHANNELS.runtimeGetInfo);
