...
import { buildCitationQuote, finalizeCitations } from "./citation-usage.util";
...
/**
 * Excerpts placed in the prompt. Deliberately independent of maxCitations:
 * the citation limit used to double as the context limit, so showing fewer
 * sources also shrank the evidence the model saw. Now the model always sees
 * up to this many excerpts and the response is trimmed by usage instead.
 */
const MAX_CONTEXT_EXCERPTS = 10;
...
export class LlmAnswerProvider implements AnswerProvider {
  ...
  async answer(input: {
    ...
    return {
      ...
      citations: finalizeCitations(
        context.citations,
        answerText,
        input.maxCitations ?? DEFAULT_MAX_CITATIONS,
      ),
    };
  }
  ...
  > {
    ...
    // The full answer is accumulated so the terminal chunk can report which
    // excerpts were actually cited ([n] markers) instead of returning every
    // prompted excerpt as a source. An empty accumulation also catches
    // providers that complete "successfully" without emitting a single token.
    let answerText = "";
    ...
    for await (const chunk of stream) {
      if (chunk.done) {
        ...
        yield {
          ...
          citations:
            answerText.length > 0
              ? finalizeCitations(context.citations, answerText, maxCitations)
              : [],
  ...
  private buildContext(results: SemanticSearchResult[]): {
    ...
    const deduped = selected
      ...
      // Cap the prompt at MAX_CONTEXT_EXCERPTS. Every numbered excerpt gets a
      // citation entry, so any [n] the model emits stays resolvable; usage-based
      // trimming afterwards decides what the client actually displays.
